...
async function loadStationSheet(station) {
  try {
    console.log('Loading rainfall cart for:', station);
    
    const response = await fetch(`https://wxbackend-production.up.railway.app/api/sheets/rainfallcart?station=${station}`);
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to load rainfall cart: ${response.status} ${errorText}`);
    }
    ...
    if (!data.sheetUrl) {
      throw new Error('No rainfall cart URL available for this station');
    }
    ...
    showAlert(`Loaded ${station}'s RAINFALL CART successfully`, 'success');
    
  } catch (error) {
    console.error('Error loading rainfall cart:', error);
...
async function saveSheetChanges() {
  try {
    ...
    showAlert('Rainfall cart changes saved successfully!', 'success');
    
  } catch (error) {
    console.error('Error saving rainfall cart:', error);

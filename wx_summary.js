...
async function loadStationSheet(station) {
  try {
    ...
    const response = await fetch(`https://wxbackend-production.up.railway.app/api/sheets/wxsummary?station=${station}`);
    ...
    showAlert(`Loaded ${station}'s WX SUMMARY successfully`, 'success');
...
async function saveSheetChanges() {
  try {
    ...
    showAlert('DOC changes saved successfully!', 'success');
